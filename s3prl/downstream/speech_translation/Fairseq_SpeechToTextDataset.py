...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        #********************************************************
        if split == "train": # 1%:2882  %5:14410   10%: 28818  100%: 288187
            proportion = 2882
            cc = list(zip(audio_paths, n_frames, speakers, ids, src_langs, tgt_langs, src_texts, tgt_texts))
            random.seed(15)
            random.shuffle(cc)
            audio_paths[:], n_frames[:], speakers[:], ids[:], src_langs[:], tgt_langs[:], src_texts[:], tgt_texts[:] = zip(*cc)

            audio_paths = audio_paths[0:proportion]
            n_frames = n_frames[0:proportion]
            speakers = speakers[0:proportion]
            ids = ids[0:proportion]
            src_langs = src_langs[0:proportion]
            tgt_langs = tgt_langs[0:proportion]
            src_texts = src_texts[0:proportion]
            tgt_texts = tgt_texts[0:proportion]
        #********************************************************
