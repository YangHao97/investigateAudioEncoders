...
class Featurizer(nn.Module):
    ...
    def transform_and_normalize(self, vecs, kernel, bias):
        ...
        # norms = (vecs**2).sum(1).view(-1, 1)**0.5
        # min_v = torch.full_like(norms, 1e-8)
        # return vecs / torch.where(norms < 1e-8, min_v, norms)
    ...
    def forward(
        ...
        raw_feature = self.tolist(paired_wavs, feature)

        w = np.genfromtxt("/home/haoy/da33_scratch/haoy/s3prl/s3prl/upstream/kernel.csv", delimiter=",")
        m = np.genfromtxt("/home/haoy/da33_scratch/haoy/s3prl/s3prl/upstream/mu.csv", delimiter=",")
        # w = torch.from_numpy(w).float().to("cuda:0")
        # m = torch.from_numpy(m).float().to("cuda:0")
        new_feature = []
        for f in raw_feature:
            vec = f.cpu().detach().numpy()
            new_vec = self.transform_and_normalize(vec, w, m)
            #new_vec = self.transform_and_normalize(f, w, m)
            new_vec = torch.from_numpy(new_vec).float()
            new_vec.requires_grad=True
            new_feature.append(new_vec.to("cuda:0"))
            #new_feature.append(new_vec)
